<template>
    <div v-if="data.show" :class="data.close ? 'hide' : 'show'" class="bg-[#0a6373] px-3 py-2 text-gray-300 rounded-lg flex items-center justify-between pop-up">
        <p class="pr-10">
            {{message}}
        </p>
        <button @click="closeNoti"
        v-if="data.done" class="w-8 h-8 rounded-full close-btn flex items-center justify-center bg-[#0000004c] hover:bg-[#0000002c]">
...
<script setup>
import { ref } from 'vue';
import itech from '../../../library/itech';
...
if(props.show){
    itech.wait(3000,()=>{},closeNoti)
}
function closeNoti(){
    itech.wait(30,function(){
        data.value.close = true
    },function(){
...
.pop-up{
    ...
    bottom: 20px;
    z-index: 999;
    box-shadow: 0 0px 5px rgba(0, 0, 0, 0.664);
    cursor: pointer;
}
.close-btn{
    display: none;
